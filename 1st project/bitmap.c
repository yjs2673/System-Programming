...
void bitmap_expand(struct bitmap *bitmap, int size) {
	ASSERT(bitmap != NULL);

	size_t prev = bitmap->bit_cnt;
	bitmap->bit_cnt += (size_t)size;
	bitmap->bits = realloc(bitmap->bits, byte_cnt (bitmap->bit_cnt + (size_t)size));
	bitmap_set_multiple(bitmap, prev, (size_t)size, false);
}
